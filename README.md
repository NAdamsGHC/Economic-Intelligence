# Gateshead Economic Intelligence Catalogue

Interactive HTML dashboards exploring open ONS datasets for **Gateshead borough** and the wider **North East of England**. Catalogued by ONS topic. An independent, unofficial project — not a Gateshead Council product.
...
The site wears **one civic-style skin** (PT Sans, `#045a90` blue, `#252525` masthead, square corners) over a River Tyne / Gateshead Quays hero, and every dashboard wears the same skin — one font, one colour scheme across the whole product line. The landing page renders from **[`catalog.json`](catalog.json)** — the single source of truth for the product list — with a **Pinned section** first (products flagged `"pinned": true`), then the ONS-topic sections, plus **live search**; every product carries a shared brand bar with a **copy-direct-link** button. To add a product: drop the HTML in its topic folder, add an entry to `catalog.json`, and paste the brand bar (`assets/brand-bar.html`) after `<body>`. Shared assets live in [`assets/`](assets/).
...
## Economy — ELS scorecard (all local indicators)
...
- Bottom UK quartile on **11** ranked indicators, top quartile on **9**: deficits cluster in pay, household income, productivity and health; strengths in learning, connectivity and affordability.
- **Skills pipeline is a top-quartile strength** — FE achievements +53% vs England, participation, apprenticeship starts and achievements all top-quartile.
- **Employment matches England (within the CI) but the work is low-value** — GVA per hour, median pay and GDHI per head all bottom-quartile.
- **Health carries the cost**: adult and child obesity, preventable CVD mortality and healthy life expectancy all adverse.
- **Affordability (4.8× earnings vs England 7.6×) and connectivity (4G ranked 1st of 361, gigabit above England) are assets.**
